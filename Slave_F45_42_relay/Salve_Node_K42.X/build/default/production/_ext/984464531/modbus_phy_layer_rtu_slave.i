...
# 21 "../modbus/modbus_phy_layer_rtu_slave.c"
...
# 21 "../modbus/modbus_phy_layer_rtu_slave.c" 2
...
# 24 "../modbus/modbus_phy_layer_rtu_slave.c" 2
...
# 25 "../modbus/modbus_phy_layer_rtu_slave.c" 2
...
# 26 "../modbus/modbus_phy_layer_rtu_slave.c" 2
...
data transfer_data;
mb mbstate;
extern BOOL flag_send_rf_485;
uint8_t flagReceiveRS485LoRa = 0;
uint8_t index_putc = 0;
uint8_t lengh_data = 0;
uint8_t lengh_data_cache = 0;
uint8_t flag_decode = 0;
uint8_t next_flag = 0;
uint8_t data_decode[128];
uint8_t flag_id_mb = 0;

extern uint32_t tick_reset_setting_rx_lora;
extern uint8_t sum_reset_device;
...
char data_buffer[20] = "";
char inputstring[20] = "";
uint8_t bufferLoRaSendData[254];
BOOL flag_data_st = 0;
BOOL setting_start = 0;
uint8_t index = 0;
uint8_t count_end = 0;
uint8_t lenRFSettingData = 0;
uint8_t dem = 0;
uint8_t number = 0;
BOOL flag_hmi_error = 0;
BOOL flag_has_data = 0;
uint8_t flagInstallRFSettings = 0;
uint8_t flagCheckRFSettings = 0;
uint8_t rfSettingResponse[15] = "";
extern volatile uint16_t timer_modbus;

extern enum _readDataSensor {
    BEGIN_MODBUS_SENSOR,
...
void modbus_check_timeout(void) {
    if (flag_has_data == 1) {
        if (modbus_timeout_enabled && (timer_modbus >=10)) {
            flag_has_data = 0;
...
void modbusReceivedDataLoRa(void) {
    ...
    readDataSensor = STOP_MODBUS_SENSOR;
    flagReceiveRS485LoRa = 0;

    char c;
    c = U2RXB;

        if (!modbus_serial_new) {
            if (c == 0x40 && next_flag == 0) {

                modbus_serial_crc.d = 0xFFFF;
                modbus_rx.error = 0;
                next_flag++;
                tick_reset_setting_rx_lora = 0;
                sum_reset_device = 0;
            }
            else if (next_flag == 1) {
                next_flag++;
            ...
            if (dem == lengh_data_cache) {

                modbus_rx.address = 0xFF;
                modbus_serial_new = TRUE;
                lengh_data = lengh_data_cache;
                next_flag = 0;
                flag_decode = 1;
