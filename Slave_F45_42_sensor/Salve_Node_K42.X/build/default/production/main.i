...
# 1 "./lora_setting.h" 1
# 66 "./lora_setting.h"
uint8_t rf_add_low = 0;

uint8_t rf_add_high = 0;
...
uint8_t rf_packet = 0;
uint8_t rf_rssi = 0;
uint8_t rf_reserve = 0;
uint8_t rf_pow = 0;

uint8_t rf_channel = 0;

uint8_t rf_rssi_ts = 0;
uint8_t rf_fixed = 0;
uint8_t rf_reply = 0;
uint8_t rf_lbt = 0;
uint8_t rf_wor = 0;
uint8_t rf_time_wor = 0;

uint8_t rf_key_low = 0;
uint8_t rf_key_high = 0;

uint8_t rfSettingsData[9];
uint8_t byte_save = 0;
# 47 "main.c" 2
...
extern uint8_t flagInstallRFSettings;
extern uint8_t flagCheckRFSettings;
extern uint8_t lenRFSettingData;
extern uint8_t rfSettingResponse[15];
extern uint8_t flagReceiveRS485LoRa;

extern float tempVal, humVal;
uint8_t key[16] = "1";
const unsigned long DELTA = 1;
...
uint8_t sum_error_get485 = 0;
unsigned long ledToggleTimeout = 0;
unsigned long sensorDataTimeout = 0;
extern volatile unsigned long tick_time1 = 0;
extern volatile unsigned short sensor_timer1_ms = 0;
uint8_t eeprom_buffer_write[10];
uint8_t eeprom_buffer_read[10];
uint8_t read_eeprom_flag = 0;
extern uint8_t flag_id_mb;
uint32_t tick_reset_setting_rx_lora;
uint8_t sum_reset_device = 0;
...
# 224 "main.c"
  ...
  do { LATDbits.LATD4 = 0; } while(0);
    ...
    NVMCON1bits.NVMREG = 0b10;
        ...
        do { LATDbits.LATD4 = 0; } while(0);
        LATAbits.LATA4 = 0;LATAbits.LATA5 = 0;_delay((unsigned long)((1000)*(20000000/4000.0)));
        ...
        __asm(" clrwdt");do { LATDbits.LATD4 = 0; } while(0);UART1_Write(0x44);UART2_Write(0x44);
             ...
             do { LATDbits.LATD4 = 1; } while(0) ;
                ...
                switch (modbus_rx.func) {
                    ...
                    case FUNC_WRITE_SINGLE_COIL:
                        if (modbus_rx.data[3] > 8)
                            ...
                            if (modbus_rx.data[2] == 0xFF) {
                                coils |= (1 << make16(modbus_rx.data[0], modbus_rx.data[1]));
                    ...
                    case FUNC_READ_HOLDING_REGISTERS:
                        if (make16(modbus_rx.data[2], modbus_rx.data[3]) <= 0 || make16(modbus_rx.data[2], modbus_rx.data[3]) > 50)
                            modbus_exception_rsp(modbus_address, modbus_rx.func, ILLEGAL_DATA_VALUE);
                        else if (make16(modbus_rx.data[0], modbus_rx.data[1]) < 0 ||
                                make16(modbus_rx.data[0], modbus_rx.data[1]) - 0 + make16(modbus_rx.data[2], modbus_rx.data[3]) > 50) {
                            modbus_exception_rsp(modbus_address, modbus_rx.func, ILLEGAL_DATA_ADDRESS);
                        } else {
                    ...
                    case FUNC_WRITE_SINGLE_REGISTER:
                        if (make16(modbus_rx.data[0], modbus_rx.data[1]) < 0 || make16(modbus_rx.data[0], modbus_rx.data[1]) > 0 + 50)
                            modbus_exception_rsp(modbus_address, modbus_rx.func, ILLEGAL_DATA_ADDRESS);
                        else {
                    ...
                    case FUNC_WRITE_MULTIPLE_REGISTERS:
                        if (make16(modbus_rx.data[0], modbus_rx.data[1]) < 0 || make16(modbus_rx.data[0], modbus_rx.data[1]) > 0 + 50)
                            modbus_exception_rsp(modbus_address, modbus_rx.func, ILLEGAL_DATA_ADDRESS);
                        else if (make16(modbus_rx.data[0], modbus_rx.data[1]) + make16(modbus_rx.data[2], modbus_rx.data[3]) > 50)
                            modbus_exception_rsp(modbus_address, modbus_rx.func, ILLEGAL_DATA_ADDRESS);
                        else {
                            uint8_t i = 0, j = 0;
